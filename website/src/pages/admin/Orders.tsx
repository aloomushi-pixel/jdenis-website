...
import type { WebsiteOrder } from '../../lib/supabase';
import { getWebsiteOrders, updateWebsiteOrderStatus } from '../../lib/supabase';

const STATUS_OPTIONS = [
    { value: 'pending', label: 'Pendiente', color: 'bg-yellow-100 text-yellow-700' },
    { value: 'completed', label: 'Pagado', color: 'bg-blue-100 text-blue-700' },
...
function OrderRow({ 
    ...
    // Parse JSON buyer_info and shipping_address
    const customer = typeof order.buyer_info === 'string' ? JSON.parse(order.buyer_info || '{}') : (order.buyer_info || {});
    const shipping = typeof order.shipping_address === 'string' ? JSON.parse(order.shipping_address || '{}') : (order.shipping_address || {});

    // For backwards compatibility or default fallback
    const fullName = customer?.fullName || customer?.name || order.user_email?.split('@')[0] || '—';
    const email = customer?.email || order.user_email || '—';
    const phone = customer?.phone || '—';
    
    // Use merchant_order_id or fallback to short id
    const orderNumber = order.merchant_order_id || order.id.split('-')[0].toUpperCase();
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-4 border border-gray-100 hover:border-gray-200 transition-colors">
            ...
            >
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="font-bold text-gray-900">#{orderNumber}</h3>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusInfo.color}`}>
                            {statusInfo.label}
                        </span>
                        {/* Indicador de Pago Rápido */}
                        {order.payment_status === 'approved' || order.status === 'completed' || order.status === 'processing' || order.status === 'shipped' || order.status === 'delivered' ? (
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-sm ml-8">
                        <div>
                            <p className="text-gray-400 text-xs">Cliente</p>
                            <p className="font-medium text-gray-800 truncate" title={fullName}>
                                {fullName}
                            </p>
                        </div>
                        <div>
            ...
            <AnimatePresence>
                {isExpanded && (
                    ...
                    >
                        <div className="p-6 md:pl-14 grid lg:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-3 bg-white p-5 rounded-xl border border-gray-200">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-700 mt-0.5 break-all">{email}</p>
                                        </div>
                                        <div>
                                    ...
                                    <div className="pt-3 border-t border-gray-100 mt-3">
                                        ...
                                        <p className="text-sm text-gray-700 mt-1 leading-relaxed">
                                            {shipping.address || 'Sin especificar'}<br/>
                                            {shipping.city || ''}{shipping.city && shipping.state ? ', ' : ''}{shipping.state || ''} {shipping.zip || ''}
                                        </p>
                                        {shipping.references && (
                                            <p className="text-xs text-gray-600 mt-2 bg-indigo-50/50 p-2.5 rounded-lg border border-indigo-100/50 flex gap-2">
                                                ...
                                                <span><strong className="font-medium text-indigo-900">Referencias:</strong> {shipping.references}</span>
                                            </p>
                                        )}
                                    </div>
                                    {order.notes && (
                            ...
                            <div>
                                <h4 className="font-serif text-lg text-gray-800 mb-4 flex items-center gap-2">
                                    ...
                                    Productos ({items.reduce((acc, curr) => acc + (curr.quantity || 1), 0)})
                                </h4>
                                ...
                                {items.length === 0 ? (
                                    ...
                                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                                        <ul className="divide-y divide-gray-100 max-h-[320px] overflow-y-auto w-full">
                                            {items.map((item, idx) => (
                                                <li key={item.id || item.product_id || idx} className="p-4 flex items-center gap-4 hover:bg-gray-50/50 transition-colors w-full">
                                                    {item.image ? (
                                                        <img 
                                                            src={item.image} 
                                                            alt={item.name} 
                                                            className="w-14 h-14 object-cover rounded-lg border border-gray-100 shadow-sm shrink-0"
                                                        />
                                                    ) : (
                                                        <div className="w-14 h-14 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-center text-gray-400 shadow-sm shrink-0">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-sm font-medium text-gray-900 truncate" title={item.name || item.product_name}>{item.name || item.product_name}</p>
                                                        <p className="text-sm text-gray-500 mt-0.5">
                                                            Cant: <span className="font-medium text-gray-700">{item.quantity}</span> × ${(Number(item.price) || item.unit_price || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                                                        </p>
                                                    </div>
                                                    <div className="text-sm font-bold text-gray-900 shrink-0">
                                                        ${((Number(item.price) || item.unit_price || 0) * (item.quantity || 1)).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                                                    </div>
                                                </li>
                                            ))}
                                        </ul>
...
export default function AdminOrders() {
    ...
    const fetchOrders = useCallback(async () => {
        setLoading(true);
        try {
            const data = await getWebsiteOrders(filter || undefined);
            setOrders(data || []);
        } catch (error) {
    ...
    const handleStatusChange = async (orderId: string, newStatus: string) => {
        ...
        try {
            await updateWebsiteOrderStatus(orderId, newStatus);
            await fetchOrders();
        } catch (error) {
