...
async function checkGroups() {
    ...
    for (const group of groups) {
        ...
        // Find the "representative" product for this group
        // Specifically, the one that might have gotten the high-res image.
        // Let's just look at all products in this group
        const groupProducts = group.product_variants.map(v => products.find(p => p.id === v.product_id)).filter(Boolean);

        // Find if any of them has a mitiendanube high-res image
        const highResProduct = groupProducts.find(p => p.image_url && p.image_url.includes('mitiendanube.com'));

        if (highResProduct) {
            console.log(`Group: ${group.name} -> Best image: ${highResProduct.image_url}`);
            const needsUpdate = groupProducts.filter(p => p.image_url !== highResProduct.image_url);
            console.log(`  -> ${needsUpdate.length} variants need this image synced.`);
            variantsToUpdate += needsUpdate.length;
        } else {
            console.log(`Group: ${group.name} -> NO high-res image found among its ${groupProducts.length} variants.`);
        }
    }
    console.log(`Total child variants needing sync: ${variantsToUpdate}`);
}
